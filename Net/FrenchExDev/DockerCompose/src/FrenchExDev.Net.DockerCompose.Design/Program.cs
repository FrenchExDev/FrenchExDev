...
var logger = loggerFactory.CreateLogger("DockerComposeScraper");

var parallel = 4;
var projectDir = AppContext.BaseDirectory;
// Navigate from bin/Debug/net10.0 → src/FrenchExDev.Net.DockerCompose.Design → src/FrenchExDev.Net.DockerCompose/scrape
var outputDir = Path.GetFullPath(Path.Combine(projectDir, "..", "..", "..", "..", "FrenchExDev.Net.DockerCompose", "scrape"));
string? minVersion = "2.20.0";
...
var collector = new GitHubReleasesVersionCollector("docker", "compose");
...
var imageTag = (string version) => $"docker-compose-scrape:{version}";
...
async Task<bool> BuildImage(string version)
{
    ...
    try
    {
        logger.LogInformation("[{Version}] Installing docker-compose in container {Id}...", version, containerId[..12]);
...
async Task<CommandTree?> ScrapeVersion(string version)
{
    ...
    try
    {
        var pipeline = new ScrapePipeline()
            ...
            .OutputTo(Path.Combine(outputDir, $"docker-compose-{version}.json"));
